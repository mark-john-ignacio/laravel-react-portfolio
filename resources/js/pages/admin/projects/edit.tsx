...
interface Project { id: number; title: string; slug: string; description: string; technologies: string[]; featured: boolean; image_url?: string; github_url?: string; live_url?: string; order: number; }

export default function EditProject({ project }: { project: Project }) {
  const { data, setData, put, processing, errors } = useForm({ ...project, technologies: project.technologies || [] });

  function submit(e: React.FormEvent) { e.preventDefault(); put(route('admin.projects.update', project.id)); }

  return (
    <AppLayout>
      <Head title={`admin/projects/${project.id}/edit`} />
      <main className="p-4">
        <h1 className="text-2xl font-bold text-green-400 mb-4">$ projects edit --id {project.id}</h1>
          ...
          <CardContent className="p-6">
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                ...
                <Input id="technologies" value={(data.technologies as string[]).join(', ')} onChange={(e) => setData('technologies', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} />
              </div>
