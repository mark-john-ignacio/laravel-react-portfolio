...
interface Experience { id: number; company: string; role: string; start_date: string; end_date?: string | null; is_current: boolean; summary?: string | null; highlights?: string[]; order: number; }
...
export default function EditExperience({ experience }: { experience: Experience }) {
  ...
  function submit(e: React.FormEvent) { e.preventDefault(); put(route('admin.experiences.update', experience.id)); }

  return (
    <AppLayout>
      <Head title={`admin/experiences/${experience.id}/edit`} />
      <main className="p-4">
        <h1 className="text-2xl font-bold text-green-400 mb-4">$ experiences edit --id {experience.id}</h1>
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 flex items-center gap-2">
                  <input id="is_current" type="checkbox" checked={!!data.is_current} onChange={(e) => setData('is_current', e.target.checked)} />
                  <Label htmlFor="is_current">current</Label>
                </div>
              </div>
              <div>
                <Label htmlFor="summary">summary</Label>
                <textarea id="summary" className="w-full rounded border bg-background p-2 font-mono" rows={4} value={data.summary || ''} onChange={(e) => setData('summary', e.target.value)} />
              </div>
              <div>
                <Label htmlFor="highlights">highlights (one per line)</Label>
                <textarea id="highlights" className="w-full rounded border bg-background p-2 font-mono" rows={4} value={(data.highlights as string[]).join('\n')} onChange={(e) => setData('highlights', e.target.value.split('\n').map(s => s.trim()).filter(Boolean))} />
              </div>
              <div>
              ...
              <div className="flex gap-3">
                ...
                <Link href={route('admin.experiences.index')} className="text-cyan-400 underline self-center">cancel</Link>
              </div>
            </form>
          </CardContent>
        </Card>
